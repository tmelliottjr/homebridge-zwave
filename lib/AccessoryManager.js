...
class AccessoryManager {
      ...
      .forEach(service => {
        switch (service) {
          ...
          case 'HumiditySensor':
            ...
            break
          case 'LeakSensor':
            this._configureLeakSensorService()
            break
          case 'LightSensor':
  ...
  /**
   * Configure the "Leak Sensor" service for the accessory
   */
  _configureLeakSensorService () {
    const service = this._getService(this._hapService.LeakSensor)
    const characteristic = service.getCharacteristic(this._hapCharacteristic.LeakDetected)

    // Setup handler for when the value for the "Leak Detected" characteristic is requested by HomeKit
    const characteristicName = 'Leak Detected'
    const zwaveNodeValueCriteria = {
      commandClass: COMMAND_CLASS_ALARM,
      valueIndex: ALARM_INDEX_WATER_LEAK
    }

    const leakDetectedValueFilter = value => value.includes('Water Leak')
    
    characteristic
      .on('get', this._makeCharacteristicGetterForZwaveNodeValue(
        characteristicName,
        this._makeZwaveNodeValueResolver(zwaveNodeValueCriteria),
        leakDetectedValueFilter
      ))
